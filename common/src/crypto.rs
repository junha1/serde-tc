//! A set of types and functions related to cryptography, that are widely used in the entire system
use std::fmt;
...
///
/// This implies that the signature is created on `Hash256::hash(serde_hdk::to_vec(T).unwrap())`.
#[derive(Debug, PartialEq, Eq, PartialOrd, Ord, Clone, Hash, Serialize, Deserialize)]
...
pub fn generate_keypair(seed: impl AsRef<[u8]>) -> (PublicKey, PrivateKey) {
    let seed_ = Hash256::hash(seed).hash.data;
...
mod tests {
    ...
    fn pretty_format() {
        ...
        assert_eq!(
            hash.to_string(),
            "47173285a8d7341e5e972fc677286384f802f8ef42a5ec5f03bbfa254cb01fad".to_owned()
        );
        assert_eq!(hash.to_string().len(), 64);
        let encoded = serde_hdk::to_string(&hash).unwrap();
        assert_eq!(
            encoded,
            "\"47173285a8d7341e5e972fc677286384f802f8ef42a5ec5f03bbfa254cb01fad\"".to_owned()
        );
        ...
        let encoded = serde_hdk::to_string(&public_key).unwrap();
        assert_eq!(encoded, "\"04d7ca91994cabd9bdb62ebc3525f0c7faa543d998f64cc4217978ad19c0a9f908bc6d47d3c9e4eb39e7472c492b106cdb49fb9d6d99cb692157bebdc9b5d842ab\"");
        assert_eq!(encoded.len(), 132);
        let encoded = serde_hdk::to_string(&private_key).unwrap();
        assert_eq!(
            encoded,
            "\"d3ad8aaebaf9bb2e86c92689cae116dfc3ca5e4759fa1eb53cd9aa873e56194a\""
        );
        assert_eq!(encoded.len(), 66);
        let signature = Signature::sign(hash, &private_key).unwrap();
        let encoded = serde_hdk::to_string(&signature).unwrap();
        assert_eq!(encoded, "\"dfc6e599dcc56d92af7acc647a03526f73842ddd4ecb045124657fd569de783d37e3e8ff59a379d2f4c5d62dd4d3ecbbd3272bf3ad0362a5bb339925c6ab84301b\"");
        assert_eq!(encoded.len(), 132);
    }
    ...
    fn hash_encode_decode() {
        let hash = Hash256::hash("hello world");
        let encoded = serde_hdk::to_string(&hash).unwrap();
        let decoded = serde_hdk::from_str(&encoded).unwrap();
    ...
    #[test]
    fn private_key_debug_output() {
        let (_, private_key) = generate_keypair("hello world");
        assert_eq!("[Censored]", format!("{:?}", private_key));
    }
}
